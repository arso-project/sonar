...
async function create1 (opts = {}) {
  let { dht } = opts
  if (!dht) dht = await createDHT()
  const swarmOpts = { bootstrap: dht.bootstrap }
  ...
  async function cleanup () {
    ...
    await rimraf(dir)
    if (!opts.dht) dht.cleanup()
  }
}

async function create2 () {
  const dht = await createDHT()
  const [workspace1, cleanup1] = await create1({ dht })
  const [workspace2, cleanup2] = await create1({ dht })
  return [workspace1, workspace2, cleanup]
...
test('basic put and get', async t => {
  // try {
  const [w1, cleanup] = await create1()
...
test('basic replication', async t => {
  const [w1, w2, cleanup] = await create2()
  const c1 = w1.Collection('default')
  await setup(c1)
